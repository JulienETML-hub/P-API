    ...
    User.findOne({ where: { username: req.body.username } }) // Recherche l'utilisateur par son pseudo
        .then((user) => { // Si l'utilisateur n'existe pas
            ...
            bcrypt // Si l'utilisateur existe
                .compare(req.body.password, user.password) // On compare le mot de passe envoyé avec celui enregistré
                .then((isPasswordValid) => {
                    if (!isPasswordValid) { // Si le mot de passe est incorrect on renvoie un message d'erreur
                        ...
                        // JWT
                        const token = jwt.sign({ userId: user.id }, privateKey, { // On signe le token avec la clé privée

...
//                     } else {
//                         // JWT
//                         const token = jwt.sign({ id_Utilisateur: Utilisateur.id }, privateKey, {
//                             expiresIn: "1y",
//                         });
//                         const message = `L'utilisateur a été connecté avec succès`;
...
//             }
//             bcrypt // Si l'utilisateur existe
//                 .compare(req.body.motDePasse, Utilisateur.motDePasse) // On compare le mot de passe envoyé avec celui enregistré
//                 .then((isPasswordValid) => {
//                     if (!isPasswordValid) { // Si le mot de passe est incorrect on renvoie un message d'erreur
//                         const message = `Le mot de passe est incorrecte.`;
//                         return res.status(401).json({ message });
//                     } else { // Si le mot de passe est correct on renvoie un message de succès et on génère un token JWT
//                         // JWT
//                         const token = jwt.sign({ id_utilisateur: Utilisateur.id }, privateKey, { // On signe le token avec la clé privée
//                             expiresIn: "1y", // Le token expire dans 1 an
//                         });
//                         const message = `L'utilisateur a été connecté avec succès`;
//                         return res.json({ message, data: Utilisateur, token }); // On renvoie le token avec les données de l'utilisateur
//                     }
//                 });
//         })
//         .catch((error) => { // Si une erreur survient on renvoie un message d'erreur
